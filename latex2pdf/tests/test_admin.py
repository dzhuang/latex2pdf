...
from latex.admin import LatexPdfAdmin
...
class AdminTest(L2ITestMixinBase, TestCase):
    ...
    def get_changelist(self, request, model=None, model_admin=None):
        model = model or models.LatexPdf
        model_admin = model_admin or LatexPdfAdmin(models.LatexPdf, site)
    ...
    def get_admin_l2p_change_list_view_url(self, model_name=None):
        model_name = model_name or models.LatexPdf.__name__
        return self.get_admin_change_list_view_url(
            app_name="latex", model_name=model_name.lower())

    def get_admin_l2p_add_view_url(self, model_name=None):
        model_name = model_name or models.LatexPdf.__name__
        return self.get_admin_add_view_url(
            app_name="latex", model_name=model_name.lower())

    def get_admin_l2p_change_view_url(self, args, model_name=None):
        model_name = model_name or models.LatexPdf.__name__
        return self.get_admin_change_view_url(
            app_name="latex", model_name=model_name.lower(), args=args)

    def test_list_view(self):
        resp = self.c.get(self.get_admin_l2p_change_list_view_url())
    ...
    def test_add_view(self):
        resp = self.c.get(self.get_admin_l2p_add_view_url())
        self.assertEqual(resp.status_code, 200)

    def test_change_view(self):
        resp = self.c.get(
            self.get_admin_l2p_change_view_url(
    ...
    def test_filter_spec_no_filter(self):
        rf = RequestFactory()
        request = rf.get(
            self.get_admin_l2p_change_list_view_url(), {})
    ...
    def test_filter_spec_no_errors(self):
        rf = RequestFactory()
        request = rf.get(
            self.get_admin_l2p_change_list_view_url(), {"has_compile_error": "n"})
        request.user = self.superuser
        changelist = self.get_changelist(request)
    ...
    def test_filter_spec_has_errors(self):
        rf = RequestFactory()
        request = rf.get(
            self.get_admin_l2p_change_list_view_url(), {"has_compile_error": "y"})
        request.user = self.superuser
        changelist = self.get_changelist(request)
